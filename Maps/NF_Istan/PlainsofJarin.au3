#include <Array.au3>

Func VQPlainsofJarin()
		Out("VQPlainsofJarin")
		Local $aWaypointsA[14][2] = [ [15474, 855] _		
		, [12657, -3190] _
		, [10998, -3788] _
		, [9517, -9223] _
		, [9225, -12080] _
		, [7371, -13130] _
		, [4658, -11377] _
		, [4348, -9673] _
		, [967, -8290] _
		, [-376, -10837] _
		, [1284, -4833] _
		, [2185, -3214] _
		, [2798, -1086] _
		, [658, -132] ]

		Local $aWaypointsB[33][2] = [ [-1167, -2506] _		
		, [56, -3581] _
		, [1205, -4359] _
		, [-2476, -4365] _
		, [-4331, -4247] _
		, [-4441, -4299] _
		, [-6103, -7388] _
		, [-6831, -5191] _
		, [-7944, -9190] _
		, [-7865, -10152] _
		, [-8603, -11971] _
		, [-9227, -10116] _
		, [-10365, -11258] _
		, [-11918, -8958] _
		, [-8778, -12778] _
		, [-11027, -12618] _
		, [-14433, -12129] _
		, [-15663, -13366] _
		, [-16660, -10345] _
		, [-19810, -8547] _
		, [-18724, -4342] _
		, [-15496, -6159] _
		, [-10988, -6357] _
		, [-7066, -5661] _
		, [-6697, -1569] _
		, [-7505, -67] _
		, [-10239, -83] _
		, [-12738, -2447] _
		, [-12854, 2247] _
		, [-12825, 2951] _
		, [-10480, 1934] _
		, [-15741, -202] _
		, [-16660, -2566] ]

		Local $aWaypointsC[57][2] = [ [-19832, 4057] _	
		, [-19542, 6489] _
		, [-19290, 8248] _
		, [-17723, 11309] _
		, [-15560, 14412] _
		, [-14549, 13487] _
		, [-14523, 11676] _
		, [-15615, 8992] _
		, [-13320, 8133] _
		, [-11479, 6381] _
		, [-11044, 10663] _
		, [-12638, 13346] _
		, [-14239, 17574] _
		, [-13482, 19292] _
		, [-8697, 18044] _
		, [-8216, 14896] _
		, [-6892, 15546] _
		, [-8086, 11741] _
		, [-6242, 14401] _
		, [-5444, 16782] _
		, [-4527, 19163] _
		, [-2393, 19037] _
		, [-2972, 15434] _
		, [1916, 14760] _
		, [3927, 15444] _
		, [5118, 14719] _
		, [8896, 17446] _
		, [10262, 15670] _
		, [7740, 11712] _
		, [8373, 10442] _
		, [10215, 11152] _
		, [12568, 11719] _
		, [13922, 12790] _
		, [14639, 9192] _
		, [15556, 7067] _
		, [14312, 6369] _
		, [12906, 7376] _
		, [14193, 3453] _
		, [12609, 2514] _
		, [10721, 1758] _
		, [8698, 651] _
		, [7792, -120] _
		, [6414, -3313] _
		, [6170, -5356] _
		, [6781, -8526] _
		, [5002, -3392] _
		, [3593, -512] _
		, [4701, 1264] _
		, [6879, 2184] _
		, [6365, 6035] _
		, [5966, 8714] _
		, [2655, 8684] _
		, [1486, 3095] _
		, [-4, 1649] _
		, [-6132, 1686] _
		, [-8150, 4112] _
		, [-8243, 8285] ]
		
		Local $aWaypoints = $aWaypointsA
		_ArrayAdd($aWaypoints, $aWaypointsB)
		_ArrayAdd($aWaypoints, $aWaypointsC)
	
		GetBlessing(18460, 1001)
		Pathfinder_MoveTo_Waypoints($aWaypointsA)
		
		GetBlessing(-1261, 3162)
		Pathfinder_MoveTo_Waypoints($aWaypointsB)
		
		GetBlessing(-17677, 3306)
		Pathfinder_MoveTo_Waypoints($aWaypointsC)
		
		Pathfinder_MoveTo_Waypoints_Reverse($aWaypoints)
        
EndFunc
